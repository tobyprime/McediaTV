...
public final class MtvWorldUiInputHook {
    ...
    private static void tick(Minecraft client) {
        if (!WorldUiCapabilityState.getInstance().supported() || client.player == null) {
            reset();
            return;
        }
...
if (selection != null && MtvWorldUiRenderer.presentation().isPlaylistExpanded()) requestVisiblePage(selection.target());
    ...
    private static Selection select(Minecraft client) {
        ...
        var screens = EntityPlayerManager.getInstance().worldUiScreens().stream()
                ...
                .toList();
        var hit = WorldUiScreenRaycast.select(new Vector3d(origin.x, origin.y, origin.z),
                new Vector3d(camera.forwardVector()), screens.stream().map(EntityPlayerHandle.WorldUiScreen::plane).toList()).orElse(null);
