...
5. 对频道 revision、列表索引、URL、速度、音量和时间位置进行最终领域校验，再调用既有服务修改状态。

屏幕命中、正面朝向和方块遮挡全部由客户端本地射线判定；服务端不重建屏幕平面，也不执行世界射线。`hitU`/`hitV` 仅保留在协议中用于兼容，服务端只做范围校验而不据此校验命中。不设置固定交互距离；只要玩家有权限、处于同一世界且目标/频道状态有效，就可以控制。
